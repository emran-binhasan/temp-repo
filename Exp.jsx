...
const Slider = () => {
	...
	useEffect(() => {
		...
		const fetchSlides = async () => {
			try {
				const response = await axios.get(`${import.meta.env.VITE_API_URL}/get-slider`, {
					params: { placement: "homeHero" },
				...
				if (isMounted && !axios.isCancel(error)) {
					...
					setSlides([]); // Reset to empty array
				}
			}
		};
	...
	return (
		<div className="relative bg-black">
			...
			{/* Post:
			{{ base_url }}/api/open-bo-account Get:
			{{ base_url }}
			{{ base_url }}/get-slider?placement=homeSecond
			{{ base_url }}/blogs
			{{ base_url }}/management
			{{ base_url }}/board-of-directors
			{{ base_url }}/notice-popup
			{{ base_url }}/download-forms Temporary base url: */}
			{/* Thumbnail Slider */}
			<div
				ref={thumbnailRef}
